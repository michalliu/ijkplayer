...
{
    av_log(NULL, AV_LOG_ERROR, "jebbe concat_probe start %s\n bufsize:%d \ncontent:\n%s", probe->filename,probe->buf_size, probe->buf != NULL?"probe buff is not null":"probe buff is null");
    int result = memcmp(probe->buf, "qzvideo version 1.0", 19) ?
           0 : AVPROBE_SCORE_MAX;
    if (result == 0 && av_strstart(probe->filename, "qzvideo:", NULL)) {
          result = AVPROBE_SCORE_MAX;
    }
    av_log(NULL, AV_LOG_ERROR, "jebbe concat_probe ret %d\n",result);
    return result;
}
...
{
    av_log(avf, AV_LOG_ERROR, "jebbe open_file\n");
    ConcatContext *cat = avf->priv_data;
...
static int concat_read_file(AVFormatContext *avf)
{
  av_log(avf, AV_LOG_ERROR, "jebbe concat_read_header\n");
  ConcatContext *cat = avf->priv_data;
  uint8_t buf[4096];
  ...
  while (1) {
      ...
      } else if (!strcmp(keyword, "qzconcat")) {
...
fail:
   av_log(avf, AV_LOG_ERROR, "jebbe concat_read_header failed\n");
  concat_read_close(avf);
  return ret;
}

static int concat_read_filename(AVFormatContext *avf)
{
  av_log(avf, AV_LOG_ERROR, "jebbe concat_read_header\n");
  ConcatContext *cat = avf->priv_data;
  uint8_t buf[4096];
...
static int concat_read_header(AVFormatContext *avf)
{
    ...
    while (1) {
        ...
        if (!strcmp(keyword, "file")) {
            ...
            avf->streams[avf->nb_streams - 1]->id =
                strtol(get_keyword(&cursor), NULL, 0);
        } else if (!strcmp(keyword, "qzvideo")) {
            char *ver_kw  = get_keyword(&cursor);
            char *ver_val = get_keyword(&cursor);
            if (strcmp(ver_kw, "version") || strcmp(ver_val, "1.0")) {
                av_log(avf, AV_LOG_ERROR, "Line %d: invalid version\n", line);
...
{
    av_log(avf, AV_LOG_ERROR, "jebbe open_next_file\n");
    ConcatContext *cat = avf->priv_data;
...
static int filter_packet(AVFormatContext *avf, ConcatStream *cs, AVPacket *pkt)
{
    ...
    //jebbe :av_assert2(cs->out_stream_index >= 0);
...
static const AVClass qzvideoformat_class = {
    .class_name = "qzvideo demuxer",
    .item_name  = av_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};

  //.long_name      = NULL_IF_CONFIG_SMALL("Virtual qzvideo concatenation script"),
AVInputFormat ijkff_qzvideoformat_demuxer = {
    .name           = "qzvideoformat",
    .long_name      = "Qzone Video Controller",
    ...
    .priv_class     = &qzvideoformat_class,
};
